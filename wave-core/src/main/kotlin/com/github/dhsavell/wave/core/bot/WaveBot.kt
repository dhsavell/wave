...
class WaveBot constructor(
    ...
    fun onMessageSent(event: MessageReceivedEvent) {
        ...
        if (message.author == client.ourUser || channel.isPrivate ||
            !message.content.startsWith(defaultCommandPrefix, ignoreCase = true)
        ) {
            return
        }

        val commandResult = commandRunner.runCommand(message.content.substring(defaultCommandPrefix.length), message, db)
