...
class PermissionManagerTest : StringSpec({
    ...
    val mockCommand = mock<CommandAction> {
    ...
    "A PermissionManager with no set override returns a CommandAction's default permission" {
        val manager = PermissionManager(db)
        manager.getEffectivePermission(mockCommand, mock()) shouldBe AnybodyCanUse
    }

    "Permission overrides can be set for a CommandAction" {
        val manager = PermissionManager(db)
